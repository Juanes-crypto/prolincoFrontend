...
import { ShieldCheckIcon, Cog6ToothIcon, ClipboardDocumentCheckIcon , LinkIcon, DocumentTextIcon, PencilIcon } from '@heroicons/react/solid';
...
const Admin = () => {
    ...
    const [editingText, setEditingText] = useState({ section: null, field: null, value: '' }); 
    ...
    const fetchContent = async () => {
        ...
        try {
            ...
            setAdminTools(initialTools.map(tool => ({
                ...tool,
                url: urls[tool.key] || '#', 
            })));
            ...
            setError("Error al cargar el contenido. Asegúrate de que el backend esté activo y el token sea válido.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const startTextEdit = (section, field, initialValue) => {
        setEditingText({
            section,
            field,
            value: Array.isArray(initialValue) ? initialValue.join(', ') : initialValue 
        });
    ...
    const handleContentUpdate = (updatedPayload) => {
        ...
        if (sectionToUpdate === 'organizacional') {
            setContent(prev => ({ ...prev, organizational: { ...prev.organizational, ...updatedPayload } }));
        } else if (sectionToUpdate === 'admin') {
            setContent(prev => ({ ...prev, admin: { ...prev.admin, ...updatedPayload } }));
        }

        // Si se actualizó una URL, actualizamos el estado de las herramientas
        if (adminTools.some(tool => tool.key === updatedKey)) {
            setAdminTools(prevTools => prevTools.map(tool => 
    ...
    if (loading && !editingText.field && !editingUrl.toolName) return <div className="text-prolinco-secondary text-center p-10 font-semibold">Cargando contenido estratégico...</div>;
    ...
    return (
        <div className="animate-fadeIn relative"> 
            ...
            <header className="mb-8 border-b border-prolinco-secondary pb-4">
                ...
                <p className="text-gray-500 mt-1">Gestión del diagnóstico general y los documentos fundamentales de la empresa.</p>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
                ...
                <Card title="Diagnóstico General" icon={Cog6ToothIcon}>
                    <p className="whitespace-pre-line">{content.admin.diagnostic || 'No definido.'}</p>
                    {isAdmin && (
                        <button 
                            onClick={() => startTextEdit('admin', 'diagnostic', content.admin.diagnostic || '')}
                ...
                <Card title="Objetivo Específico" icon={ClipboardDocumentCheckIcon }>
                    <p className="whitespace-pre-line">{content.admin.specificObjective || 'No definido.'}</p>
                     {isAdmin && (
                        <button 
                            onClick={() => startTextEdit('admin', 'specificObjective', content.admin.specificObjective || '')}
                            className="mt-4 text-sm text-prolinco-primary hover:text-prolinco-secondary font-semibold flex items-center"
                        >
                            <PencilIcon className="h-4 w-4 mr-1" /> Editar
                        </button>
                ...
                >
                    ...
                    <p className="text-sm italic">{content.organizational.mission || 'No definida.'}</p>
                    
                    <h4 className="font-bold text-prolinco-primary mt-4">Visión:</h4>
                    <p className="text-sm italic">{content.organizational.vision || 'No definida.'}</p>
                    
                    <h4 className="font-bold text-prolinco-primary mt-4">Valores:</h4>
                    <div className="flex flex-wrap gap-2 mt-1">
                        {(content.organizational.corporateValues || []).map((value, index) => (
                            <span key={index} className="px-3 py-1 bg-prolinco-dark text-prolinco-primary text-xs font-medium rounded-full">
                                {value}
                            </span>
                        ))}
                    </div>
                    {isAdmin && (
                        ...
                        <button 
                            onClick={() => startTextEdit('organizacional', 'corporateValues', content.organizational.corporateValues || [])}
                     ...
                     {isAdmin && (
                        <button 
                            onClick={() => startTextEdit('organizacional', 'mission', content.organizational.mission || '')}
