...
import { LockClosedIcon, DocumentIcon, UserIcon, ArrowRightIcon } from '@heroicons/react/24/solid';
...
const AuditPage = () => {
    ...
    useEffect(() => {
        const fetchLogs = async () => { // Renombramos la interna para claridad
            setLoading(true);
            setError(null);
    ...
    const renderActionBadge = (actionType) => {
        ...
        switch (actionType) {
            ...
            case 'DOC_UPLOAD': color = 'bg-yellow-100 text-yellow-800'; Icon = DocumentIcon; break;
            case 'DOC_DELETE': color = 'bg-red-100 text-red-800'; Icon = DocumentIcon; break;
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo de Acción</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {logs.map((log) => (
                                    <tr key={log._id}>
                                        <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(log.createdAt)}</td>
                                        <td className="px-3 py-4 whitespace-nowrap text-sm">
                                            {renderActionBadge(log.actionType)}
                                        </td>
                                        <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {log.user ? `${log.user.name} (${log.user.role})` : 'Usuario Desconocido'}
                                        </td>
                                        <td className="px-3 py-4 text-sm text-gray-700 max-w-lg truncate">{log.description}</td>
                                        <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-400">{log.ipAddress || 'N/A'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
