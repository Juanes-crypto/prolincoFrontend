...
const UserManagement = () => {
    // 🌟 1. OBTENER ROL DEL CONTEXTO EN LUGAR DE LOCALSTORAGE 🌟
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            if (_err.response?.status === 403) {
                 setError("Error de Backend: Permisos insuficientes para acceder a esta lista.");
            } else {
                 setError(_err.response?.data?.message || "Error al cargar la lista de usuarios. Verifica la ruta del backend '/users'.");
            }
    ...
    // 3. Guardar el Nuevo Rol (Mismo código, correcto)
    const handleRoleUpdate = async (e) => {
        ...
        try {
            ...
            setEditingUser(null);
            setNewRole('');
            alert(`Rol de ${editingUser.name} actualizado a ${newRole}.`);

        } catch (_err) {
            setError(_err.response?.data?.message || "Error al actualizar el rol del usuario.");
    ...
    if (loading) return <div className="text-prolinco-secondary text-center p-10 font-semibold">Cargando lista de usuarios...</div>;
    if (error) return <div className="text-red-600 text-center p-10 font-semibold">{error}</div>;
    ...
    return (
        <div className="animate-fadeIn relative"> 
            ...
            <header className="mb-8 border-b border-prolinco-primary pb-4">
                ...
                <p className="text-gray-500 mt-1">Administración de permisos de acceso a las diferentes plataformas internas.</p>
            </header>
                        ...
                        {users.map((user) => (
                            <tr key={user._id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${user.role === 'admin' ? 'bg-red-100 text-red-800' : user.role === 'talento' ? 'bg-blue-100 text-blue-800' : user.role === 'servicio' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'}`}>
